...
class BlackjackTournament {
	...
	private async playPlayerHand(
		...
		try {
			const hand = this.game.getPlayerHand(playerId);
			...
			while (handValue < 17) {
				...
				if (hand.length === 2 && handValue >= 9 && handValue <= 11) {
					const currentBet = this.game.getPlayerBet(playerId);
					if (player.currentBalance >= currentBet) {
				...
				const newHand = this.game.getPlayerHand(playerId);
				const newValue = this.game.getPlayerHandValue(playerId);
				console.log(
					`New hand: ${this.formatHand(newHand)} (Value: ${newValue})`,
				);

				if (newValue >= 21) break;
			}

			if (!this.game.isPlayerBusted(playerId) && handValue < 21) {
				console.log(`${player.name} stands`);
				this.game.stand(playerId);
			}
		} catch (error) {
			console.log(`${player.name} encountered an error:`, error.message);
		}
	}
	...
	private updatePlayerStats(
		...
		for (const [playerId, player] of this.players) {
			...
			if (gamePlayer) {
				...
				// Simple win/loss tracking (this would need to be more sophisticated in a real implementation)
				if (gamePlayer.balance > player.currentBalance) {
					player.handsWon++;
				} else {
					player.handsLost++;
				}
...
async function runExample() {
	const tournament = new BlackjackTournament(20);

	// Add players
	tournament.addPlayer("alice", "Alice", 1000);
	tournament.addPlayer("bob", "Bob", 1000);
	tournament.addPlayer("charlie", "Charlie", 1000);
	tournament.addPlayer("diana", "Diana", 1000);
